...
// Store the original counts for display in tabs
$products_count = 0;
$stores_count = 0;

if (!empty($search_query)) {
    // Search products
    if ($search_type === 'all' || $search_type === 'foods') {
        $products_query = "SELECT p.*, r.name as restaurant_name, r.restaurant_image_url,
                          ...
                          LEFT JOIN product_reviews pr ON p.id = pr.product_id
                          WHERE p.is_available = 1 
                          AND (p.name LIKE ? OR p.description LIKE ? OR p.category LIKE ? OR r.name LIKE ?)
                          GROUP BY p.id
                          ORDER BY p.created_at DESC";
        $products_stmt = $conn->prepare($products_query);
        $search_param = "%$search_query%";
        $products_stmt->bind_param("ssss", $search_param, $search_param, $search_param, $search_param);
        $products_stmt->execute();
        $products_result = $products_stmt->get_result();
        ...
        while ($row = $products_result->fetch_assoc()) {
            ...
            $search_results['products'][] = $row;
        }
        $products_stmt->close();
    }
    
    // Search stores - FIXED: Removed description from search since it doesn't exist in restaurants table
    if ($search_type === 'all' || $search_type === 'shops') {
        $stores_query = "SELECT * FROM restaurants 
                        WHERE is_active = 1 
                        AND (name LIKE ? OR category LIKE ?)";
        $stores_stmt = $conn->prepare($stores_query);
        $search_param = "%$search_query%";
        ...
        while ($row = $stores_result->fetch_assoc()) {
            ...
            $search_results['stores'][] = $row;
        }
        $stores_stmt->close();
    }
    
    // Get counts for ALL results regardless of current filter
    $all_products_query = "SELECT COUNT(*) as count FROM products p 
                          JOIN restaurants r ON p.restaurant_id = r.id 
                          WHERE p.is_available = 1 
                          AND (p.name LIKE ? OR p.description LIKE ? OR p.category LIKE ? OR r.name LIKE ?)";
    $all_products_stmt = $conn->prepare($all_products_query);
    $all_products_stmt->bind_param("ssss", $search_param, $search_param, $search_param, $search_param);
    $all_products_stmt->execute();
    $all_products_result = $all_products_stmt->get_result();
    $products_count_data = $all_products_result->fetch_assoc();
    $products_count = $products_count_data['count'] ?? 0;
    $all_products_stmt->close();
    
    $all_stores_query = "SELECT COUNT(*) as count FROM restaurants 
                        WHERE is_active = 1 
                        AND (name LIKE ? OR category LIKE ?)";
    $all_stores_stmt = $conn->prepare($all_stores_query);
    $all_stores_stmt->bind_param("ss", $search_param, $search_param);
    $all_stores_stmt->execute();
    $all_stores_result = $all_stores_stmt->get_result();
    $stores_count_data = $all_stores_result->fetch_assoc();
    $stores_count = $stores_count_data['count'] ?? 0;
    $all_stores_stmt->close();
    
    $search_results['total_count'] = $products_count + $stores_count;
}
...
$stores_js = [];
foreach ($search_results['stores'] as $store) {
    $stores_js[$store['id']] = [
        'name' => $store['name'],
        'description' => $store['category'] ?? 'Restaurant',
...
<head>
    ...
    <title>Search Results - UMAK Foodhub</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/7.0.1/css/all.min.css">
    <style>
        /* All the CSS styles from UserInt.php including enhanced review styles */
        /* ... (include all the CSS styles from the reference page) ... */
...
<body class="bg-gray-50 min-h-full">
            ...
            <span class="text-xl font-bold umak-yellow-text ml-1">Foodhub</span>
                ...
                <div class="hidden md:flex items-center flex-1 max-w-2xl mx-8">
                    <div class="search-container w-full">
                        <form id="search-form" action="search.php" method="GET" class="flex items-center w-full bg-white rounded-lg shadow-sm border border-gray-300 overflow-hidden focus-within:ring-2 focus-within:ring-yellow-400 focus-within:border-yellow-400 transition-all duration-200">
                            <select name="type" class="search-dropdown px-4 py-3 bg-transparent border-none text-sm focus:outline-none focus:ring-0 appearance-none cursor-pointer">
                                ...
                                <option value="foods" <?php echo $search_type === 'foods' ? 'selected' : ''; ?>>Foods</option>
                                <option value="shops" <?php echo $search_type === 'shops' ? 'selected' : ''; ?>>Stores</option>
                            </select>
                            <div class="w-px h-6 bg-gray-300"></div>
                            <input type="text" name="q" placeholder="What are you craving today?" value="<?php echo htmlspecialchars($search_query); ?>" class="search-input flex-1 px-4 py-3 border-none focus:outline-none focus:ring-0 text-sm" id="search-input" autocomplete="off">
        ...
        <div class="p-6">
            ...
            <div class="md:hidden mb-6">
                <div class="search-container">
                    <form id="mobile-search-form" action="search.php" method="GET" class="flex items-center bg-white rounded-lg shadow-sm border border-gray-300 overflow-hidden focus-within:ring-2 focus-within:ring-yellow-400 focus-within:border-yellow-400 transition-all duration-200">
                        ...
                        <input type="text" name="q" placeholder="Search food or stores..." value="<?php echo htmlspecialchars($search_query); ?>" class="flex-1 px-3 py-3 border-none focus:outline-none focus:ring-0 text-sm" id="mobile-search-input">
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <?php if (!empty($search_query)): ?>
                                <p class="text-lg opacity-90">Showing results for "<?php echo htmlspecialchars($search_query); ?>"</p>
                            <?php else: ?>
                                <p class="text-lg opacity-90">Search for food or stores</p>
                            <?php endif; ?>
                        </div>
            ...
            <?php if (!empty($search_query)): ?>
                ...
                <div class="search-results-count">
                    Found <?php echo $search_results['total_count']; ?> result<?php echo $search_results['total_count'] !== 1 ? 's' : ''; ?> for "<?php echo htmlspecialchars($search_query); ?>"
                </div>

                <!-- Search Tabs -->
                <div class="search-tabs">
                    <button class="search-tab <?php echo $search_type === 'all' ? 'active' : ''; ?>" onclick="changeSearchType('all')">All Results</button>
                    <button class="search-tab <?php echo $search_type === 'foods' ? 'active' : ''; ?>" onclick="changeSearchType('foods')">Foods (<?php echo $products_count; ?>)</button>
                    <button class="search-tab <?php echo $search_type === 'shops' ? 'active' : ''; ?>" onclick="changeSearchType('shops')">Stores (<?php echo $stores_count; ?>)</button>
                </div>

                <!-- Search Results Content -->
                <?php if ($search_results['total_count'] > 0): ?>
                    <!-- Products Results -->
                    <?php if (($search_type === 'all' || $search_type === 'foods') && !empty($search_results['products'])): ?>
                    ...
                    <!-- Stores Results -->
                    <?php if (($search_type === 'all' || $search_type === 'shops') && !empty($search_results['stores'])): ?>
                    <section class="search-results-section">
                        <h3>Restaurants & Stores</h3>
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            <?php foreach ($search_results['stores'] as $store): 
                            ...
                            <div class="store-card bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden transition-all duration-300 cursor-pointer relative" 
                                ...
                                <div class="p-4">
                                    ...
                                    <p class="text-sm text-gray-600 mb-2 line-clamp-2"><?php echo htmlspecialchars($store['category'] ?? 'Restaurant'); ?></p>
                ...
                <?php else: ?>
                    ...
                    <div class="no-results">
                        ...
                        <p class="text-gray-500 mb-4">We couldn't find any matches for "<?php echo htmlspecialchars($search_query); ?>"</p>
                        
                        <!-- Search Suggestions -->
                        <div class="search-suggestions">
                            <h4>Try these suggestions:</h4>
                            <div class="suggestion-tags">
                                <div class="suggestion-tag" onclick="searchSuggestion('Chicken')">Chicken</div>
                                <div class="suggestion-tag" onclick="searchSuggestion('Rice')">Rice</div>
                                <div class="suggestion-tag" onclick="searchSuggestion('Burger')">Burger</div>
                                <div class="suggestion-tag" onclick="searchSuggestion('Pasta')">Pasta</div>
                                <div class="suggestion-tag" onclick="searchSuggestion('Drinks')">Drinks</div>
                                <div class="suggestion-tag" onclick="searchSuggestion('Dessert')">Dessert</div>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
            <?php else: ?>
                ...
                <div class="no-results">
                    <div class="no-results-icon">🔍</div>
                    <h3 class="text-xl font-semibold text-gray-700 mb-2">Search for food or stores</h3>
                    <p class="text-gray-500 mb-4">Enter a search term above to find delicious food and restaurants</p>
    ...
    function changeSearchType(type) {
        ...
        const typeInput = searchForm.querySelector('select[name="type"]');
